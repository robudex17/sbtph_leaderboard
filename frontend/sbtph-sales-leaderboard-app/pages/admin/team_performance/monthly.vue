pages/admin/agent_performance/monthly.vue<template>
    ...
    <div v-if="leaderBoardStore.state.loading">
      ...
      <h1 class="text-2xl font-extrabold text-gray-800 mb-3 text-center">Team Monthly Performance  Information</h1>
            ...
            >
              ...
              <td class="py-0.5 px-2 border text-center text-xs">
                <div class="flex justify-center space-x-2" v-if="parseFloat(agent.final_ratings) > 0">
                  ...
                  >
                    Team Monthly Performance Details
                  </NuxtLink>


                </div>
                 <span v-else class="text-red-500 font-bold">NO  Team Monthly Performance Details</span>
              </td>
            </tr>
          </tbody>
        </table>
  ...
  import { setRatingNameColor, setRatingColor } from '@/utils/constants'
...
const leaderBoardData = (leaderboardOption,query, year_summary) => {
  leaderBoardStore.fetchLeaderboard(leaderboardOption,route.query, year_summary);
};
...
//   const agents = computed(() => manageSalesAgentStore.state.salesAgents);
  ...
  const SelectedTeamMembers = (team) => {
     ...
     navigateTo({path:'/team_performance/month', query: {team_id: team.team_id, month: team.month, year: team.year, leaderboardOption: 'team' }})
 ...
 // newRoute.query.leaderboardOption = leaderboardOption
  //newRoute.query.year_summary = year_summary
